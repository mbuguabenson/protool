...
export class AutonomousBotEngine {
    ...
    private async analyzeMarket(symbol: string): Promise<MarketAnalysis> {
        try {
            ...
            const variance =
                history.prices.reduce((sum, price) => sum + Math.pow(price - mean, 2), 0) / history.prices.length;
    ...
    private async executeTrade(signal: TradeSignal) {
        try {
            ...
            if (result.profit > 0) {
                ...
                this.log(
                    `LOSS: ${signal.strategy} -$${Math.abs(result.profit).toFixed(2)} | Martingale x${multiplier}`
                );
            }
