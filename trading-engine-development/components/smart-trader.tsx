...
export function SmartTrader({ theme = 'dark', currency = 'USD' }: SmartTraderProps) {
    ...
    return (
        <div
            className={`space-y-4 p-6 rounded-2xl backdrop-blur-xl transition-all ${
                theme === 'dark'
        ...
        >
            <div className='flex items-center justify-between'>
                ...
                <Badge className={connectionStatus.includes('✅') ? 'bg-green-500' : 'bg-yellow-500'}>
                    {connectionStatus}
                </Badge>
            </div>
                    ...
                    >
                        <table className='w-full text-xs'>
                            <thead className={`sticky top-0 ${theme === 'dark' ? 'bg-[#0f1629]/50' : 'bg-gray-100'}`}>
                                <tr>
                                    <th
                                        className={`p-2 text-left ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}
                                    >
                                        Time
                                    </th>
                                    <th
                            ...
                            <tbody>
                                {tradeLogs.map(log => (
                                    ...
                                    >
                                        ...
                                        <td
                                            className={`p-2 font-bold ${log.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}
                                        >
                                            {log.profit >= 0 ? '+' : ''}
                                            {log.profit.toFixed(2)}
                                        </td>
                                        <td
                                            className={`p-2 font-bold ${log.result === 'WIN' ? 'text-green-400' : 'text-red-400'}`}
                                        >
                                            {log.result}
                                        </td>
                                    </tr>
