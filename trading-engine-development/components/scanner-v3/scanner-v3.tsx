...
import { DERIV_APP_ID, DERIV_API } from '@/lib/deriv-config';
...
interface ScannerV3Props {
    isFloating?: boolean;
    onClose?: () => void;
}

export default function ScannerV3({ isFloating = false, onClose }: ScannerV3Props) {
    ...
    const runScan = async () => {
        ...
        const server = DERIV_API.WEBSOCKET_LEGACY.replace('wss://', '').replace('/websockets/v3', '');
        ...
        try {
            const ws = new WebSocket(`wss://${server}/websockets/v3?app_id=${DERIV_APP_ID}`);
  ...
  <block type="after_purchase" id="scanner_after_purchase" x="667" y="0">
    ...
    const handleDownloadBot = () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `AI_Scanner_${activeSignal.name}_${activeSignal.signalText}.xml`;
            a.click();
            URL.revokeObjectURL(url);
        } catch (e) {
            console.error('Failed to generate bot', e);
        } finally {
                                                ...
                                                {r.overUnderPredictions.map((pred, pi) => (
                                                    ...
                                                    >
                                                        <div className='ou-pred-header'>
                                                            ...
                                                            {pi === 0 && <span className='ou-best-badge'>★ Best</span>}
                                                        </div>
                                                        <div className='ou-pred-stats'>
                                                            ...
                                                            <div className='ou-stat'>
                                                                ...
                                                                <span className='ou-stat-value'>{pred.expected}%</span>
                                                            </div>
                                                            <div className='ou-stat'>
                ...
                <div className='scanner-v3__cta-section'>
                    <div className='best-signal-highlight'>
                        ...
                        <div className='highlight-content'>
                            ...
                            <span className='highlight-signal'>{bestSignal.signalText}</span> ({bestSignal.confidence}%
                            Conf)
                        </div>
                        <div className='highlight-entry'>
                            ...
                            {getTrendIcon(bestSignal.entryPoint.trendDirection)} {bestSignal.entryPoint.trendDirection}
                        </div>
                    </div>
                    <div className='cta-button-group'>
                        ...
                        <button onClick={handleDownloadBot} className='btn-primary' disabled={isLoadingBot}>
                            {isLoadingBot ? 'Generating...' : 'Download Bot XML'}
                        </button>
                    </div>
                </div>
            )}
