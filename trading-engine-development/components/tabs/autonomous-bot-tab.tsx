...
export function AutonomousBotTab({ theme = 'dark', symbol }: AutonomousBotTabProps) {
    ...
    const { apiClient, isConnected, isAuthorized, balance, isLoggedIn } = useDerivAPI();
        ...
        const runTrade = async () => {
            try {
                ...
                const shouldContinue =
                    isActive &&
                    totalLossRef.current < maxLosses &&
                    stats.pl < profitTarget &&
                    newPL > -maxLosses * roundedStake;
        ...
        <div className='space-y-6'>
            ...
            >
                <CardContent className='pt-6 flex items-center justify-between'>
                    <div className='flex items-start gap-3'>
                        <Zap
                            className={`w-5 h-5 shrink-0 ${theme === 'dark' ? 'text-purple-400' : 'text-purple-600'}`}
                        />
                        <div>
            ...
            >
                ...
                <CardContent className='space-y-4'>
                    <div>
                        <label
                            className={`text-sm font-semibold block mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
                        >
                            Select Contracts
                        </label>
                            ...
                            {[
                                ...
                                >
                                    <input
                                        ...
                                        onChange={e =>
                                            setSelectedContracts(prev => ({ ...prev, [key]: e.target.checked }))
                                        }
                                        className='w-4 h-4'
                                    />
                ...
                <CardContent className='space-y-4'>
                    <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
                        ...
                        >
                            ...
                            <p
                                className={`text-lg font-bold mt-2 ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}
                            >
                                {stats.losses}
                            </p>
                        </div>
            ...
            <Card className={theme === 'dark' ? 'bg-blue-500/10 border-blue-500/30' : 'bg-blue-50 border-blue-200'}>
                <CardContent className='pt-6'>
                    <div className='flex gap-3'>
                        <AlertCircle
                            className={`w-5 h-5 shrink-0 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}
                        />
                        <div>
                            ...
                            <p className={`text-sm mt-1 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'}`}>
                                This bot trades DIFFERS contract and multiple OVER/UNDER combinations. It will
                                auto-restart after each trade until it reaches your profit target, max loss limit, or
                                you stop it manually. Ensure your API token is connected.
                            </p>
                        </div>
                    </div>
                </CardContent>
