...
export function useActiveSymbols(
    ...
    const loadContractsFor = useCallback(
        async (wsInstance: DerivWS, symbol: ActiveSymbol) => {
            const response = await wsInstance.send<ContractsForResponse>({
                contracts_for: symbol.underlying_symbol,
            });

            const filtered =
                response.contracts_for?.available?.filter(c => contractTypes.includes(c.contract_type)) ?? [];
