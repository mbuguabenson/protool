...
function formatContractType(contractType: string, labels: Record<string, string>, barrier?: string): string {
...
function ClosedPositionRow({
    ...
    return (
        <TableRow>
            <TableCell className='font-medium'>{formatContractType(pos.contract_type, contractTypeLabels)}</TableCell>
            <TableCell className='text-muted-foreground'>{getSymbolDisplayName(pos.underlying_symbol)}</TableCell>
            <TableCell className='text-right'>{pos.buy_price.toFixed(2)}</TableCell>
            <TableCell className='text-right'>{pos.sell_price.toFixed(2)}</TableCell>
...
function ProfitCell({
    ...
    return (
        <TableCell className={cn('text-right font-semibold', isProfit ? 'text-green-600' : 'text-destructive')}>
            {isProfit ? '+' : ''}
            {profit.toFixed(2)}
            {currency ? ` ${currency}` : ''}
