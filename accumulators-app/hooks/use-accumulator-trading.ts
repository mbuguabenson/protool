...
import type { DerivWS, ActiveSymbol, Tick, BuyResult } from '@deriv/core';
...
export type UseAccumulatorTradingParams = Pick<
    UseBaseTradingParams,
    'ws' | 'isConnected' | 'isExhausted' | 'isAuthenticated' | 'onAuthWSFailed'
>;

export function useAccumulatorTrading({
    ws,
    isConnected,
    isExhausted,
    isAuthenticated,
    onAuthWSFailed,
}: UseAccumulatorTradingParams): UseAccumulatorTradingReturn {
    const {
        ws: tradingWs,
    ...
    const growthRateOptions = useMemo(() => {
        const accuContract = (contracts as unknown as AccumulatorContractInfo[]).find(c => c.contract_type === 'ACCU');
