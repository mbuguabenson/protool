...
export function getDurationOptions(contracts: ContractInfo[]): DurationOption[] {
    ...
    for (const contract of contracts) {
        ...
        if (expiryType === 'tick') {
            ...
            if (!optMap.has('end-time'))
                optMap.set('end-time', { unit: 'end-time', label: DURATION_LABELS['end-time'], min: dMin, max: dMax });
        }
    }
...
export function getCloseTimeForDate(data: TradingSymbolData, date: Date): string {
    ...
    for (const event of data.events) {
        if (!event.descrip.toLowerCase().includes('closes early')) continue;
        const applies = event.dates
            .split(',')
            .map(s => s.trim())
            .some(s => s === dayNamePlural || s === dateStr);
        if (!applies) continue;
