...
export default class ActiveSymbols {
    ...
    processActiveSymbols() {
        return this.active_symbols.reduce((processed_symbols, symbol_obj) => {
            // Use underlying_symbol if available, otherwise fall back to symbol
            ...
            const isExistingValue = (object, prop) => Object.keys(object).findIndex(a => a === symbol_obj[prop]) !== -1;

            if (!isExistingValue(processed_symbols, 'market')) {
                processed_symbols[symbol_obj.market] = {
                    display_name: symbol_obj.market_display_name,
                    submarkets: {},
                };
            }

            const { submarkets } = processed_symbols[symbol_obj.market];

            if (!isExistingValue(submarkets, 'submarket')) {
                submarkets[symbol_obj.submarket] = {
                    display_name: symbol_obj.submarket_display_name,
                    symbols: {},
                };
            }

            const { symbols } = submarkets[symbol_obj.submarket];

            if (!symbols[symbol_key]) {
                symbols[symbol_key] = {
                    display_name: display_name_key,
                    pip_size: `${symbol_obj.pip}`.length - 2,
                    is_active: !symbol_obj.is_trading_suspended && symbol_obj.exchange_is_open,
                };
            }
    ...
    isSymbolClosed(symbol_name) {
        return this.active_symbols.some(
            active_symbol =>
                (active_symbol.underlying_symbol === symbol_name || active_symbol.symbol === symbol_name) &&
