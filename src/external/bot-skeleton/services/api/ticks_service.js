        ...
        if (api_base.api) {
            const subscription = api_base.api.onMessage().subscribe(({ data }) => {
                if (data.msg_type === 'tick') {
                    const { tick } = data;
                    const tick_symbol = tick.underlying_symbol || tick.symbol;
                    const { id } = tick;
                ...
                if (data.msg_type === 'ohlc') {
                    const { ohlc } = data;
                    const ohlc_symbol = ohlc.underlying_symbol || ohlc.symbol;
                    const { granularity, id } = ohlc;
                    if (this.candles.hasIn([ohlc_symbol, Number(granularity)])) {
                        this.subscriptions = this.subscriptions.setIn(['ohlc', ohlc_symbol, Number(granularity)], id);
                        const address = [ohlc_symbol, Number(granularity)];
