...
export function generateSignals(a: AnalysisResult): Signal[] {
    return [evenOddSignal(a), overUnderSignal(a), matchesSignal(a), differsSignal(a), riseFallSignal(a)];
}
...
export function generateProSignals(a: AnalysisResult): Signal[] {
    return [proEvenOddSignal(a), proOverUnderSignal(a), under7Signal(a), over2Signal(a)].filter(
        s => s.status !== 'NEUTRAL' || s.probability >= 55
    );
}
...
export function generateSuperSignals(a: AnalysisResult): Signal[] {
    ...
    return all.filter(s => s.probability >= 55 && s.status !== 'NEUTRAL').sort((a, b) => b.probability - a.probability);
}
