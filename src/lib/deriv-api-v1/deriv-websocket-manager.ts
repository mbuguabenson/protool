...
export class DerivWebSocketManager {
    ...
    private async fetchWithAuth(path: string, options: RequestInit = {}, retryCount = 0): Promise<any> {
        ...
        console.log(`[v0] REST Headers: Authorization=${this.accessToken ? 'Bearer ' + this.accessToken.substring(0, 10) + '...' : 'NONE'}, App-ID=${this.getActiveAppId()}`);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`[v0] REST error response: ${response.status} - ${errorText}`);
                try {
            ...
            console.error(`[v0] REST fetch error: ${errMsg}`);
            console.error(`[v0] This may be a CORS issue, network error, or invalid token. URL: ${url}`);
