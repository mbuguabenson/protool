...
export default class ClientStore {
    ...
    setAccountList = (account_list?: TAuthData['account_list']) => {
        this.accounts = {};
        const balanceAccounts: Record<string, { balance: number; currency: string; loginid: string }> = {};
        account_list?.forEach(account => {
            this.accounts[account.loginid] = account;
            if ((account as any).balance !== undefined) {
                balanceAccounts[account.loginid] = {
        ...
        if (Object.keys(balanceAccounts).length > 0) {
            this._all_accounts_balance = {
                accounts: balanceAccounts,
                balance: parseFloat((this.accounts[this.loginid] as any)?.balance || '0'),
                currency: this.currency,
                loginid: this.loginid,
            } as any;

            const activeAcc = this.accounts[this.loginid];
            if (activeAcc) {
                this._balance = (activeAcc as any).balance?.toString() ?? '0';
            }
        }
    };
