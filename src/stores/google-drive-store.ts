...
export default class GoogleDriveStore {
    ...
    initialise = () => {
        if (!this.discovery_docs) {
            console.warn('Google Drive discovery docs are missing. Google Drive integration loader skipped.');
            return;
        }
        try {
            gapi.load('client:picker', () => gapi.client.load(this.discovery_docs));
        } catch (error) {
            console.error('Failed to load gapi client picker:', error);
        }
    };
    ...
    initialiseClient = () => {
        if (!this.client_id) {
            console.warn('Google Drive Client ID is missing. Google Drive integration disabled.');
            return;
        }
        try {
            ...
            console.error('Failed to initialize google accounts token client:', error);
        }
    };
    ...
    async signOut() {
        if (this.access_token) {
            try {
                ...
                if (localStorage.getItem('google_access_token') && window?.google?.accounts?.oauth2) {
                    await window.google.accounts.oauth2.revoke(this.access_token);
                    localStorage.removeItem('google_access_token');
                }
            } catch (error) {
                console.error('Failed to sign out of Google Drive client:', error);
            }
            this.access_token = '';
        }
