...
export function CurrentPriceDisplay({ tick, prices, activeSymbol, pipSize }: CurrentPriceDisplayProps) {
    ...
    return (
        <div className='text-center py-6'>
            ...
            <div className='flex items-center justify-center gap-3'>
                <span className='text-4xl font-mono font-bold tracking-wide text-foreground'>{priceStr}</span>
                {(isUp || isDown) && (
                    <span
                        className={cn('text-2xl font-bold leading-none', isUp ? 'text-green-500' : 'text-destructive')}
                    >
                        {isUp ? '▲' : '▼'}
                    </span>
                )}
